...
@GetMapping("/update-difficulty")
    ...
    //2) ...
@GetMapping("/search")
    public List<Task> searchTasks (@RequestParam String username,
                                   @RequestParam String title){
        //Просто вызываем наш новый метод из репозитория
    return taskRepository.findByOwnerUsernameAndTitleContainingIgnoreCase(username, title);
}
    //поиск задачи по введенной topic
    //1) добавили в TaskRepository - List<Task> findByTopicIgnoreCase (String topic);
    //2) ...
@GetMapping("/by-topic")
public List<Task> getTasksByTopic (@RequestParam String username,
                                   @RequestParam String topic){
        return taskRepository.findByOwnerUsernameAndTopicIgnoreCase(username, topic);
}

@GetMapping("/filter")
public List<Task> filterTasks (@RequestParam String username,
                               @RequestParam String topic,
                               @RequestParam Status status){
     return taskRepository.findByOwnerUsernameAndTopicIgnoreCaseAndStatus(username, topic, status);
}
@GetMapping("by-status-sorted")
public List<Task> getByStatusSorted (@RequestParam String username,
                                     @RequestParam Status status){
        return taskRepository.findByOwnerUsernameAndStatusOrderByLevelDesc(username, status);
}
